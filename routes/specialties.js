...
router.get('/specialties/coffee/:id', async (req, res) => {
    const specialties = await Specialty.query().select().where({
    ...
    if (specialties.length > 0) {
        ...
        return res.status(400).send({
            response: 'There were no specialties of the coffee found.'
        });
    }
...
router.get('/specialties/country/:id', async (req, res) => {
    ...
    if (specialties.length > 0) {
        ...
        res.status(400).send({
            response: 'There was no specialty of the country found.'
        });
    }
